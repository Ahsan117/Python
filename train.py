...
def train_models_and_evaluate(X_train, y_train, X_test, y_test):
    ...
    account_sid = 'AC8ec38665cc31720d62306b0643d764d8'
    auth_token = 'c5d84bb5c80820ba6d229b04c94a5224'
    twilio_phone_number = '+13237460920'
    receiver_phone_number = '+917398827578'  # Receiver's phone number
    ...
    # Example usage
    if malicious_counts[winner] > 0:
        message = f'Malware detected! {malicious_counts[winner]} malicious files detected by {winner} algorithm.'
        send_sms_notification(receiver_phone_number, message)

# Prompt user to input the file path for data.csv
file_path = input("Enter the path to the data.csv file: ")
...
selected_features = data.drop(['Name', 'md5', 'legitimate'], axis=1).columns[model.get_support()]

X_train, X_test, y_train, y_test = train_test_split(
    X_new, y, test_size=0.2)
malicious_count = np.sum(y_test == 1)
print(f'Number of Malicious Files: {malicious_count}')

features = []

# Train models and evaluate performance
train_models_and_evaluate(X_train, y_train, X_test, y_test)
